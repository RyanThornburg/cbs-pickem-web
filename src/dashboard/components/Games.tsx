...
const gameDate = (gameStart: number) => {
    const game = dayjs(gameStart)
    return game.format('ddd, MMM DD')
}

const gameTime = (gameStart: number) => {
    const game = dayjs(gameStart)
    return game.format('hh:mm A')
}


export default function Games({week}: Props){
    ...
    const gameStarts = currentGames.map(item => gameDate(item.starts_at)).filter((value, index, self) => self.indexOf(value) === index)  

    // has to be a better way than this
    const gameArrays: Game[][] = []
    gameStarts.forEach((startTime: string) => {
        console.log(startTime)
        const gameBlock = currentGames.filter((game: Game) => gameDate(game.starts_at) === startTime)
